...
把一个本地文件夹变成 Gitee / GitHub 上的仓库——不用先去网页建空库，不用记
`git remote add`，不用配凭据助手。剩下的它做完：建远端仓库 → 写 `.gitignore` →
提交 → 推送 → **核对远端 SHA 确认真的推上去了**。

不装技能也行：直接跑 `python git_sync.py`，它问你四个问题，答案给完就一路到底。
...
想把一个**已经存在**的仓库（比如 Gitee 上的）也放到另一个平台？clone 用原生 git 做，
本工具负责建库 + 推送（它**不做克隆**）：

```bash
git clone https://gitee.com/<你>/<仓库>.git <目录>
python scripts/git_sync.py --dir <目录> --platform github \
    --account https://github.com/<你>/ --name <仓库> --public --branch <分支> --yes
```

四个必须注意的点：

1. **分支名要跟上**。别想当然认为都是 `main`——老仓库常常是 `master`。先 `git branch` 看一眼。
2. **`--dry-run` 在这种场景下几乎空转**：clone 下来工作区干净，暂存区 0 个文件，
   而**密钥体检是针对暂存区做的**——所以它这次什么都没扫。**必须另外扫一遍**：
   ```bash
   python scripts/check_clean.py --dir <目录>
   ```
3. **可见性照搬源仓库**（源是公开就 `--public`），否则镜像出来两边不一样。
4. **推完之后，原来的上游保持不变**——从 Gitee clone 下来的仓库本来就跟踪 `origin`，
   推一次 GitHub 不会把它改成 `mirror`，所以裸跑 `git pull` 拉的还是 Gitee。
   以后的同步用 `--status` / `--pull` / `--align`，两个平台会一起照顾到。
...
| 网络抖动 | 分级重试：直连 → 直连+HTTP/1.1 → 清代理 → 清代理+HTTP/1.1 → 绑 IP（仅 GitHub，逐个试），**任一边报出网络问题就继续降级**；网络类的失败会如实说清是哪一环出的问题，**不会拿"看起来正常的那条输出"当失败原因** |
| 令牌看起来失效了 | 有一类"认证失败"其实是**本地代理**冒充的（代理拒绝连接时 git 会借这句报错）。环境里有代理时不会因此就停，而是继续降级试下去——因为摘掉代理往往就好了。**遇到报认证失败，先查代理，别急着换令牌** |
...
| 推送**报失败**，错误却是 `Everything up-to-date` | ⚠️ **核对远端的错误被丢掉了**：push 那一步通了，而核对用的 `ls-remote` 被代理拦了（`CONNECT tunnel failed`）。老版本只信 push 的输出，就用这句"看着没事"的话报了失败，而且因为不像网络错误，**自救级别一个都不试** | 升级到 v1.13.0。自诊：`git ls-remote <远端URL> refs/heads/<分支>` 看是不是代理在拦；或临时 `unset https_proxy`。**别去换令牌** |
| **从别的平台 clone 下来镜像过去后，`git pull` 拉的是哪儿？** | 老版本推完会把上游**无条件**改成新推的远端（如 `mirror`）——裸跑 `git pull` 就默默换了平台 | 升级到 v1.13.0：已有上游**保持不动**，并打印「原本跟踪 `origin`，保持不动」。手动还原：`git config branch.<分支>.remote origin` |
| **镜像一个已有仓库时，`--dry-run` 说「暂存区是空的，没有任何文件会被推送」** | 不是错误：clone 下来工作区本来就干净，而**密钥体检是针对暂存区做的**——所以它这次其实**什么都没扫** | 另外单独扫一遍：`python scripts/check_clean.py --dir <目录>`。**别把"空转的体检"当成"体检通过"** |
...
**把已有仓库镜像到另一个平台**（clone 由你用原生 git 做，本工具负责建库 + 推送）·
**不顶掉仓库原有的上游**

**不做**：**克隆远端仓库到本地**（`git clone` 即可，本工具不代劳）· 合并分叉（该 merge 还是
rebase 由你决定）· **force push（一行都没有）** · 替你 stash 或丢弃改动 ·
处理分支合并冲突 · 删仓库或改仓库设置 · 改全局 git 配置 ·
一次处理多个分支 · Git LFS ·
