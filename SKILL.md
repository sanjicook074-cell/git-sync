...
| 做 | 不做 |
|---|---|
| **四问向导**：直接运行即可，不必记参数 | 合并分叉（该 merge 还是 rebase 由用户决定） |
| `git init`（分支 main）+ 生成 .gitignore | **force push —— 一行都没有** |
| 调 API 建远端仓库（`auto_init=false`，复用已存在的） | 替用户 stash / 丢弃工作区改动 |
| 写本仓库 `user.name` / `user.email`（local，不动 global） | 处理分支合并冲突 |
| 提交 + 推送（HTTPS 令牌 / SSH） | 删仓库、改仓库设置 |
| **拉取最新（`--pull`，只做快进）** | 改全局 git 配置 |
| **查同步状态（`--status`，含双平台是否一致）** | 一次处理多个分支 |
...
> **我能做什么**：把这个文件夹变成一个 Gitee / GitHub 仓库并推上去。建库、写 `.gitignore`、提交、推送都我来——**你不用先去网页建一个空库**。推送前我会先体检密钥和大文件。推完之后也管：看本地和远端差几个提交、把最新的拉下来、把两个平台对齐。
>
> **我需要你提供 4 样东西**：
...
### 1.4 同步模式：`--status` / `--pull` / `--align`（v1.12.0）

推完之后的三件事。**不走向导**——目标目录、平台、账号、仓库名全都能从
「现有仓库的 remote 地址 + 配置」里推出来，再问一遍纯属折磨人。

```bash
python git_sync.py --status              # 只读：差几个提交、两平台是否一致
python git_sync.py --pull                # 拉取最新（只做快进）
python git_sync.py --align               # 双平台对齐，只补推落后的那一方
python git_sync.py --pull --align        # 拉完顺手对齐
```

- **退出码**：0 一致 / 5 有差异（需要人决定）/ 6 平台拿不到（状态未知）。
- **仓库名从远端地址里认**，不靠目录名。目录名和远端仓库名不一样时这一点很关键（§7.11）。
- 完整设计规则见 **§2.7**（十条），那里是改这块代码前必读的部分。
...
| **5** | **同步模式专用：有差异 / 分叉 / 工作区不干净** | 见 `--status` 的输出提示；**这是"需要人决定"，不是错误** |
| **6** | **同步模式专用：平台都拿不到（网络或令牌）** | 状态未知，**不是"没同步"**——先解决连通性再重试 |
| 124 | 子进程超时 | 网络问题，检查代理/连通性 |

⚠️ **5 和 6 必须分开**。把"连不上"混进"不一致"里，用户会以为代码真的没同步，
去删本地文件重来；把"不一致"说成"连不上"，用户会一直重试网络。
**这两种误报的代价不对称**，所以宁可多一个码。
...
## 2.7 同步三件套：状态 / 拉取 / 对齐（v1.12.0）

**什么时候用**（用户话术 → 命令）：

| 用户说 | 用 |
|---|---|
| "同步了吗 / 有没有同步 / 落后几个提交 / 两个平台一致吗" | `--status` |
| "把最新的拉下来 / 更新一下 / 我在另一台机器上改过 / 网页上改过" | `--pull` |
| "两个平台不一样 / 补一下 GitHub / 对齐" | `--align` |
| "同步一下"（仓库已配好，只是日常推） | **什么都不加，正常跑**（见 §1.2） |

三者可以叠加，**执行顺序固定**：先拉 → 再对齐 → 最后一定报一次状态。

### 改这块之前必须知道的十条

1. **判断"谁新谁旧"只看合并基是不是祖先**，不看提交数、不看时间戳。
   `git rev-list --left-right --count <远端>...<本地>` 的输出是「左=落后、右=领先」，
   **这个顺序是反直觉的，测试里有专门一条钉住它**（接反了会把"落后"当"领先"，
   然后拿旧代码去覆盖远端）。
2. **`--pull` 只做快进**（`merge --ff-only`）。分叉就停，把 merge/rebase 的选择权交回用户。
3. **`--align` 永不 force push**。只推"远端确实是本地祖先"的那一方和"远端还没这个分支"的。
   对方有本地没有的提交 → **拒绝**。
4. **工作区不干净就拒绝拉取**，且**不替用户 stash**。丢改动这件事必须用户自己决定。
5. **必须区分「连不上」和「连上了但远端没这个分支」**。为此专门有 `ls_remote_branch()`
   返回"连通性"，而旧的 `read_remote_sha()` 两种情况都返回空串（推送场景够用，
   状态查询会把"远端还没推过"误报成"网络不通"）。退出码也因此分开：6=拿不到，5=有差异。
6. **fetch 只写 `FETCH_HEAD`**，不写 `refs/remotes/**`。原因有两个：副作用更小；
   而且本机 git 写那批 ref 会**静默失败**（§2 第 15 条）。
7. **每个平台必须用自己那个仓库名去 resolve 远端地址**。踩过：用全局的目录名 → 地址匹配不上
   → 凭空新建一个指向错误地址的 remote，还改歪了 `branch.main.remote`。详见 §7.11。
8. **状态查询的降级阶梯比推送短**（GitHub 绑 IP 只取 2 个候选）。推送失败值得等，
   查个状态拖 7 分钟就过分了。
9. **最后一定报一次状态**，哪怕只是 `--pull`。用户刚操作完最想知道的恰恰是"现在同步了没"。
   （第一版只在 `--status` 时报，于是 `--pull` 跑完静默结束，像没说清楚。）
10. **`ensure_git_available()` 只能在 `run_sync_modes()` 里调一次**，不能放进 `sync_collect()`——
    后者会被调用 2~3 次（拉取后、对齐后各重探一次），放进去会把「git version ...」重复打印。
    实测就是这么发现的。

### 只读性保证

`--status` 和 `--align` 的**判断阶段**全程只读（网络 ls-remote + fetch 到 FETCH_HEAD），
`test_sync_modes.py` §6 有一条断言：连跑 status + align 判断后，
**HEAD 与工作区文件一字未变**。只有 `--pull` 的快进和 `--align` 的推送会动东西。
...
### 7.11 第七次：补上"拉 + 状态 + 对齐"（2026-09-23，v1.12.0）

**任务**：用户说"把同步的 git 功能都加上"。**先划范围再动手**——四组候选摆出来让他挑，
他只选了「拉取与同步状态」这一组。（另外三组：分支标签 / 历史撤销 / 远端仓库管理+克隆。）

**判断**：不加限制地"全都加上"会把它变成另一个 git 客户端，丢掉"聊两句就完事"的卖点。
用户自己也选了最小的一组——**这个决定是对的**。

**做法**：三个命令，全部**只读或只增**，判断"谁新谁旧"只看**合并基是不是祖先**。
设计细则见 §2.7（十条）。

**真机演练**（不是只跑桩测试）——在 Gitee + GitHub 上建一次性仓库，故意造出
**「Gitee 领先、GitHub 落后」**的局面，然后逐步验证：

| 步骤 | 做法 | 预期 | 实际 |
|---|---|---|---|
| 1 | 建仓库 + 推两平台 | 全一致，退出码 0 | ✅ 0 |
| 2 | 用 Gitee API（`PUT /contents`，带 `sha`）**只改 Gitee 上的文件** | — | ✅ Gitee 多一个提交 `edeb6e28` |
| 3 | `--status` | gitee 本地落后 1、github 一致，退出码 5 | ✅ 5 |
| 4 | `--pull` | 快进到 `edeb6e28`，**文件内容真的变成 Gitee 那版** | ✅ 内容已核对 |
| 5 | `--align` | 只补推 GitHub | ✅ 两端一致，退出码 0 |

**演练抓出两个真 bug，都是桩测试抓不到的：**

**bug 1（测试自己抓到的）**：`classify_platform` 忘了把 `token` 放进结果字典。
而**测试夹具 `R()` 里也没有 token**——两边同时缺同一个字段，测试就永远发现不了。
→ 加了一条「夹具的键必须 ⊇ 真实现的键」的断言，拿真实现的输出对夹具。
→ 教训：**手写夹具会腐烂**。夹具形状必须由真实现来校验，不能靠人记。

**bug 2（真机演练才抓到的，更值钱）**：`align_execute` 用 `ctx["repo"]`（=**目录名**）
当全局仓库名去 `resolve_remotes`。演练目录叫 `drill`，远端仓库叫 `git-sync-drill`，
地址匹配不上 → **凭空新建了 `github-remote → .../drill.git`**，
还把 `branch.main.remote` 指到了这个假 remote 上。

最阴的是：**推送本身是对的**（`push_platform` 用的是 `r["repo"]` 拼的显式 URL），
所以屏幕上显示"补推成功"，只有一行 `[OK] 远端 github-remote → .../drill.git` 露了马脚。
后果是**推对了、remote 配错了**——用户之后裸跑 `git push` 会推到不存在的仓库。
→ 修复：每个平台各用**自己那个仓库名** resolve 一次；`ctx["repo"]` 也改成优先用
「从远端地址里认出来的真名」，只在认不出来时才退回目录名。
→ 加了 5 条回归断言（`test_sync_modes.py` §8）。
→ 教训：**"操作成功了"和"副作用对了"是两件事。**
只看成功提示会漏掉它顺手写坏的东西——**真机演练必须回头检查它留下的痕迹**
（这次是 `git remote -v` 和 `git config branch.main.remote`）。
桩测试永远不会告诉你这些，因为它根本没碰真的 `.git/config`。

**其余两个顺手修的小问题**：
- 中文表格**列对不齐**：`f"{x:<20}"` 数的是字符数，汉字占 2 列。加了 `pad()` 按显示宽度算
  （`unicodedata.east_asian_width`）。
- `--pull` 跑完**什么都不显示**：原设计只在 `--status` 时报状态。改成最后一定报一次。
- 「工作区脏」和「远端不同步」原本混成一句"有差异"，用户不知道该提交还是该拉取，**拆开说**。
