...
20. **「绑 IP」这一级必须用 DNS 当前解析出的地址，写死的老段会帮倒忙。**
    ...
    现在 `github_ips()`：**DNS 给出结果就只用 DNS 的**（取前 2 个），
    DNS 完全拿不到才退回写死的那几个。写死的 IP 是"最后手段"，不是首选。
...
21. **到 `github.com` 的 HTTP/2 会被中途 RST，切 HTTP/1.1 能立刻恢复。**
    ...
    **别去改 DNS、别去调代理、别怀疑令牌**——先试 `http.version=HTTP/1.1`。
    脚本已把它做成第 2 级（见 §2.5）。注意这**不是**性能优化，平时不要默认开。
...
| 绑了 IP 反而更慢/更不通 | 绑的是写死的老 IP 段，与本机 DNS 解析出的段不同 | v1.7.0 起 `github_ips()` 以 DNS 结果优先；写死段只在 DNS 全线拿不到时才用（§2 第 20 条） |
...
⚠️ **路由冲突（2026-09-22 复查发现）**：`github-workflow` 的 **description 和正文自相矛盾**——
...
并声明"从本地目录到远端仓库一律用本技能"。若以后又出现选错技能的情况，
先看是不是这三个技能被重新安装/更新后覆盖了描述。
...
### 7.3 远程建库用例（`python scripts/test_repo_create.py`，31 项全过）
...
### 7.4 向导与取值链路（`python scripts/test_wizard.py`，96 项全过）
...
`test_repo_create.py` 31）。版本 v1.6.0 → v1.7.0（随后的分发清理为 **v1.8.0**，见 §8）。
...
- **许可证状态如实写"未附带"**，并说明未加许可证 = 默认保留所有权利。
  不替用户选许可证，那该是他自己的决定。
